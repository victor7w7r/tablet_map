use evdev::{EventType, InputEvent, KeyCode, KeyEvent, RelativeAxisCode};
use std::io::Result;
...
impl Pad {
  pub fn new() -> Result<Self> {
    Ok(Pad {
      virt_dev: crate::utils::setup_virtual_device(
        ...
        &Some(vec![RelativeAxisCode::REL_WHEEL]),
      )?,
      scroll_threshold: 3,
      last_scroll: 0,
  ...
  pub fn exec(&mut self) -> Result<()> {
    let mut pad_dev = crate::utils::scan_dev(self.dev_name).expect("Failed to find pad device");
    pad_dev.grab()?;
